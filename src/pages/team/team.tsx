...
export default function TeamForSalesReps() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      await dispatch(fetchSalesRepsByPayloadRequest({ teamId, userId: result.payload.id }));
    }
  ...
  useEffect(() => {
    async function fetchData() {
      if (resSalesRep.data?.success) {
        await dispatch(fetchSalesRepsByPayloadRequest({ teamId, userId: currentUser?.id }));
        await dispatch(fetchSalesRepFailure(null));
      } else {
      ...
      <div className="library-page">
        <div className="row">
          <div className="col-lg-12 col-md-12 col-sm-12">
            <div className="sales-rep-wrapper">
              ...
              <div className="row teams-list-wrapper">
                {salesReps.map((item: any) => (
                  <div className="col-lg-3 col-md-4 col-sm-12" key={item.id}>
                    <SalesRepPreview item={item} userId={currentUser?.id} />
                  </div>
                ))}
              </div>
            </div>
